...
单测/e2e 仍用 **mock facilitator**（28/28 全绿，不依赖真链）。**真 v2 线载荷字段的保真**由
**计划 3 执行器的 LIVE 冒烟**（真 CDP 签名 → 真 `/facilitator/verify`，用 @x402/core+@x402/evm v2）证实。
计划 5 仅需把网关↔真 facilitator 端到端接上（真 `/verify` + 真 `/settle` 上链、funded settler），据真链回归。
